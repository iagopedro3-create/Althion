...
describe('google ads attribution calculation', () => {
  it('correctly aggregates ad spend, clicks, conversions and calculates metrics', () => {
    ...
    // Totals
    expect(result.totalAdSpend).toBe(800.0); // 480 + 320
    ...
    expect(result.costPerConsultation).toBe(400.0); // 800 / 2

    // Coverage & Uncertainty ratios
    expect(result.coverageRatio).toBe(0.6); // 3 / 5
    expect(result.uncertaintyRatio).toBe(0.2); // 1 / 5 (l-4 is organic google search without ad click id)
  });
