...
- O rate limit implementado hoje é **por endereço de cliente**, 100 req/min, global na API. O cliente é resolvido a partir do `X-Forwarded-For` **somente** quando `TRUST_PROXY_HOPS > 0` — atrás da Vercel, `1`. Com `0` atrás de um proxy, todos os clientes compartilham um balde e o limite vira teto agregado; a API avisa no boot quando isso acontece em produção. Duas limitações permanecem registradas: (a) o contador é **em memória por instância**, então em serverless o teto efetivo é `limite × instâncias` — um store compartilhado é decisão de infraestrutura ainda não tomada; (b) o balde é por endereço, então uma clínica inteira atrás de um IP público compartilha cota — bucketing por identidade autenticada exigiria rodar o throttler depois do `JwtAuthGuard`. Enquanto isso, o controle vale como contenção de rajada, não como defesa contra abuso distribuído.
...
- As áreas do web que exigem sessão são declaradas em `apps/web/src/lib/protected-areas.ts` (`AUTHENTICATED_AREAS`), consumidas pelo proxy e casadas por segmento — `/app` não protege `/application`. Uma área nova entra na barreira por uma linha nessa lista, e não por uma edição na lógica de sessão. A barreira do proxy é a camada externa: cada layout de área continua validando com `getUser()` e a API revalida o JWT por JWKS, de modo que nenhuma das três depende das outras.
