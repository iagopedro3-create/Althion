...
> **`jose` fica na major 5 enquanto o build da API for CommonJS.** A v6 é ESM-only; o `nest build` emite CommonJS, e o runtime serverless da Vercel recusa `require()` de um módulo ESM com `ERR_REQUIRE_ESM` — foi o que derrubou o 1º deploy que chegou a rodar (01/08/2026), em `/health/*`, antes de qualquer rota de negócio. O defeito **não reproduz localmente**: o Node 24 aceita `require()` de ESM, então `node dist/main.js` sobe aqui e falha lá. A v5 é dual-package (`exports['.'].require`), então o import estático continua normal. Um teste em `jwt-verifier.service.test.ts` fixa a invariante e quebra se alguém subir a major. Alternativas descartadas: `import()` dinâmico (o TypeScript o reescreve para `require` sob `module: CommonJS`), `new Function('return import(s)')` (funciona no Node, mas quebra no contexto VM do vitest — código não testável) e `module: Node16` (cascata de `resolution-mode` e extensões por toda a API).
