    ...
    <main className="portal-main">
      <header className="page-heading">
        <div>
          ...
          <p className="lead-copy">
            Acompanhe o retorno sobre o investimento publicitário conectando cliques de anúncios à
            jornada do paciente no CRM.
          </p>
        </div>
      </header>
      ...
      {campaigns.length === 0 ? (
        <section className="state-card" style={{ marginBottom: '24px' }}>
          ...
          <p className="lead-copy" style={{ marginBottom: '16px' }}>
            A conta do Google Ads está configurada. Execute a sincronização dos dados do sandbox
            para popular o dashboard de atração de leads.
          </p>
        ...
        <>
          ...
          >
            <section className="state-card" aria-label="Atribuição e Incerteza">
              ...
              <p
                className="lead-copy"
                style={{ marginTop: '16px', color: 'var(--muted)', fontSize: '0.85rem' }}
              >
                A margem de incerteza indica leads que vieram da busca do Google, mas não puderam
                ser correlacionados a cliques específicos de campanhas (ex. bloqueio de cookies,
                tráfego orgânico ou opt-out de privacidade).
              </p>
            </section>
            ...
            <section className="state-card danger" aria-label="Relação Prudente">
              ...
              <div
                style={{ display: 'flex', alignItems: 'baseline', gap: '8px', margin: '12px 0' }}
              >
                <span style={{ fontSize: '2.5rem', fontWeight: 'bold', color: 'var(--danger)' }}>
                  R$ {summary.attribution.costPerConsultation.toFixed(2)}
                </span>
                <small style={{ color: 'var(--muted)' }}>
                  Custo Real por Consulta Concluída (CAC)
                </small>
              </div>
              <ul
                className="cockpit-sla-list"
                style={{ display: 'grid', gap: '12px', fontSize: '0.9rem' }}
              >
                <li style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              >
                <small style={{ fontWeight: '600', display: 'block', marginBottom: '4px' }}>
                  ℹ️ AVISO DE CONVERSÃO
                </small>
                <small>
                  Não confunda conversões de marketing (pixels de cliques ou contatos de chat
                  iniciados) com consultas concluídas. Das leads geradas via anúncios, apenas **
                  {summary.attribution.matchedConsultationsCount}** concluíram consultas, elevando o
                  custo real de aquisição por consulta médica (CAC) para **R${' '}
                  {summary.attribution.costPerConsultation.toFixed(2)}**.
                </small>
              </div>
            </section>
          </div>
          ...
          <section
            ...
            aria-label="Campanhas de Anúncios"
            style={{ marginBottom: '24px' }}
          >
            <h2>Campanhas Ativas ({campaigns.length})</h2>
            <div
              className="portal-table-container"
                  ...
                  {campaigns.map((camp) => (
                    <tr key={camp.id} style={{ borderBottom: '1px solid var(--line)' }}>
                      ...
                      <td style={{ padding: '12px 8px' }}>
                        <span
                          className={`badge ${camp.status === 'ENABLED' ? 'success' : 'muted'}`}
                        >
                          {camp.status === 'ENABLED' ? 'Ativa' : 'Pausada'}
                        </span>
                      </td>
                      <td style={{ padding: '12px 8px' }}>
                        R$ {(camp.budget_micros / 1000000).toFixed(2)}
                      </td>
                      <td
                        style={{ padding: '12px 8px', color: 'var(--muted)', fontSize: '0.85rem' }}
                      >
                        {camp.campaign_id}
                      </td>
                    </tr>
