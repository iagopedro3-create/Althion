...
export default async function QualityPage({
  ...
  const criteria = activeRubric
    ...
    : [
        { id: 'response_time', name: 'Tempo de resposta', description: 'Agilidade no atendimento', maxScore: 5 },
        { id: 'tone_empathy', name: 'Tom e empatia', description: 'Cortesia e profissionalismo', maxScore: 5 },
        { id: 'opportunity_id', name: 'Identificação de oportunidade', description: 'Mapear a necessidade do lead', maxScore: 5 },
        { id: 'compliance', name: 'Conformidade de roteiro', description: 'Uso de termos corretos', maxScore: 5 },
        { id: 'operational_sanity', name: 'Sanidade operacional', description: 'Ausência de promessas sem base', maxScore: 5 },
      ];
      ...
      <header className="page-heading">
        <div>
          ...
          <p className="lead-copy">
            Analise interações administrativas, atribua notas de conformidade e revise flags clínicos de urgência.
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '24px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <section className="state-card" aria-label="Avaliar Conversa">
            ...
            <p className="lead-copy" style={{ marginBottom: '16px' }}>
              Rubrica ativa: <strong>{activeRubric?.title ?? 'Rubrica Padrão'}</strong> (v{rubricVersion})
            </p>
          ...
          <section className="state-card" aria-label="Sinalizar Conteúdo Clínico">
            ...
            <p className="lead-copy" style={{ marginBottom: '16px' }}>
              Se notar que o paciente está tentando obter diagnóstico ou prescrição médica, sinalize para handoff clínico.
            </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <section className="state-card" aria-label="Handoffs Clínicos">
            <h2>Handoffs Clínicos em Aberto ({flags.filter((f) => f.status !== 'resolved').length})</h2>
            {flags.filter((f) => f.status !== 'resolved').length === 0 ? (
              <p>Nenhuma conversa pendente de resolução clínica no momento.</p>
            ) : (
              <ul className="cockpit-incident-list" style={{ display: 'grid', gap: '16px', listStyle: 'none', padding: 0 }}>
                {flags
                  .filter((f) => f.status !== 'resolved')
                  .map((flag) => (
                    <li key={flag.id} className="cockpit-incident-item warning" style={{ padding: '16px', border: '1px solid var(--line)', borderRadius: '12px', background: 'var(--surface)' }}>
                      <header style={{ display: 'flex', justifyContent: 'between', alignItems: 'center', marginBottom: '8px' }}>
                        <span className="badge warning">Pendente</span>
                        <strong style={{ marginLeft: '8px' }}>Conversa: {flag.conversation_id}</strong>
                      </header>
                      <p style={{ color: 'var(--text)', margin: '8px 0' }}>{flag.flag_reason}</p>
                      <small style={{ color: 'var(--muted)', display: 'block', marginBottom: '12px' }}>
            ...
            {evaluations.length === 0 ? (
              ...
              <ul className="cockpit-sla-list" style={{ display: 'grid', gap: '12px' }}>
                {evaluations.map((evalItem) => (
                  <li key={evalItem.id} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: '600' }}>
                      <span>Conversa: {evalItem.conversation_id}</span>
                      <span className="badge success">{evalItem.total_score}%</span>
                    </div>
                    {evalItem.feedback ? (
                      <p style={{ margin: 0, fontStyle: 'italic', fontSize: '0.9rem', color: 'var(--muted)' }}>
                        "{evalItem.feedback}"
                      </p>
                    ) : null}
                    <small style={{ color: 'var(--muted)' }}>
                      Avaliado em: {formatDateTime(evalItem.created_at)} (Rubrica v{evalItem.rubric_version})
                    </small>
                  </li>
                ))}
              </ul>
