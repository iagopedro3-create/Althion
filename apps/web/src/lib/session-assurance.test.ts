...
describe('describeSessionAssurance', () => {
  ...
  it('explains, on aal1, that enrolment does not exist yet instead of offering an action', () => {
    expect(describeSessionAssurance('aal1').summary).toContain('ainda não está disponível');
  });
});
